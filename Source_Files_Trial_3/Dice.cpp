...
using namespace std;
   ...
   */
	...
	Dice::Dice(int numOfDice, int numOfSidesDie, int modifyValue) {
   ...
   */
	...
	void Dice::setNumOfDice(int numOfDice) {
   ...
   */
	...
	void Dice::setNumOfSidesDie(int numOfSidesDie) {
   ...
   */
	...
	void Dice::setModifyValue(int modifyValue) {
   ...
   */
	void Dice::setDiceValue(char* diceValue) {
   ...
   */
	int Dice::roll(char* inputDiceString) {
   ...
   * If the user chooses to roll a dice, they are prompted to enter a dice string,
	...
	int Dice::diceMenu() {
		...
		while(runMainLoop) { //Run the main part of the method
				
			clearConsole();
			cout << "Dice Demonstration\n1) Roll a dice\n2) Exit Menu\n\nEnter Option: "; //Prompt user for option
			cin >> mainMenuOption;
			...
			switch(mainMenuOption) {
				...
				case 1:
					...
					while(runInnerLoop) {
		     		...
		     		cout << "Enter the desired dice string (Format: xdy±z, where x, y and z are numbers): ";
		     			
		     		cin >> diceInput; //Take in dice input
		     		
		     		diceInputCharP = new char[diceInput.length() + 1];
		     		strcpy(diceInputCharP, diceInput.c_str()); //Cast the char* string to a cpp String
	...
	*/
	int Dice::deconstructDiceValue(char* diceVal) {
